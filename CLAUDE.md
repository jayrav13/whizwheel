...
> **Adoption note (#169).** The QA agent is newly adopted **in `ci-monitor`'s stead** — this is its first live trial. Open refinements tracked in **#169**: the DB facet is currently *opportunistic* (often `N/A` unless the dev DB is populated — possibly rehome the persistence check to a CI test), the result format becoming a fixed table, shot-posting to the PR, and the **opus + vision cost on every PR** (accepted for now). `ci-monitor` remains a registered agent (its CI logic lives on inside the QA CI facet) — if the QA agent proves unfit, reverting this section to dispatch `ci-monitor` + the main-thread visual gate is the fallback.
