...
- **GitHub issue mechanics → `github-agent`.** Dispatch `github-agent` to **create + label + dedupe** issues (it runs a dedupe scan first and applies the taxonomy above), to produce the **Now/Later + effort backlog summary**, and to **rebase stale/behind branches**. The caller (main thread or PM) supplies the intent/prose; the agent handles the GitHub conventions. It never merges, never closes issues, never pushes to `main`. (A quick one-off with no dup risk may still be filed directly — but batch/dedupe-sensitive filing and branch rebasing should route through it. PR *creation* is not its job yet — see its definition.)
