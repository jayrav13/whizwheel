...
**Concurrency cap — wave the fan-out (~3–4 heavy agents at a time).** Backgrounding is the default, but the *number* of heavy agents (Opus build / QA agents) dispatched **concurrently** is capped: keep it to **~3–4 at once**, never ~7+. iteration-0007's 2× fan-out fired ~7 concurrent heavy build agents simultaneously and tripped a transient **server-side API rate-limit that killed them all mid-run** — recovery was possible but costly. So **wave the fan-out**: dispatch a batch of ~3–4, and as each finishes, fire the next from the queue. This caps the heavy-agent count, not the orchestration discipline — you still stay ahead of the airplane, you just feed the runway in waves.
