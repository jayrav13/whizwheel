...
- **Visual gate (every PR).** `ci-monitor` reports *status* but is Bash-only — it can't *see*. After CI completes on **any** PR, the **main thread** pulls the deterministically-generated screenshots with **`bin/ci-screenshots --pr <n>`** (downloads the `ui-screenshots` artifact, prints the dir) and **reviews them against `docs/DESIGN.md`** before merge. Do this **regardless** of whether the diff looks UI-related — it's cheap, and the CI/Linux render is a useful cross-platform check (a proxy for "what a non-Mac visitor sees") that can catch incidental regressions a code diff hides. (Scale-up: a vision-capable review subagent so image tokens stay out of the main context.)
