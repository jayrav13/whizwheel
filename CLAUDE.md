...
- **`project-manager-agent`** (Opus) — project tracking. Owns `docs/` and the **issue backlog's content** (authors issue bodies / `spec:v1`, decides what gets built and the sequencing); issue **mechanics** (create/label/dedupe) route through `github-agent`. Never writes app code or agent definitions; stewards `PRODUCT.md`. Runs a full-context ingestion at every launch.
...
- **`github-agent`** (Sonnet) — GitHub mechanics. Creates issues (applies labels + dedupe scan), serves an issue's spec/state on demand, summarizes the backlog (Now/Later + effort), and rebases stale/behind branches so CI re-runs. The caller supplies intent/prose; it handles GitHub conventions. Never writes app code, agent definitions, or docs; never merges; never pushes to `main`. (Report/action-only — no worktree.) *(PR creation is intentionally out of scope for now — may be added later.)*
...
**Concurrency cap — at most 2 subagents at a time, with a standing queue.** Backgrounding is the default, but the *number* of subagents running **concurrently is hard-capped at 2** — never a third in flight. The orchestration discipline is unchanged (stay ahead of the airplane, keep every unblocked task moving); what's capped is only how many run *at once*. The mechanism is a **queue, not a wait**:

- **Maintain a standing queue.** As work is identified — from the user's direction, from the plan, from what you know must follow — **enqueue every task you know you'll need to run**, in dependency order. The queue is explicit and proactive: you decide *now* what runs next, not when an agent happens to finish.
- **Only ever 2 in flight.** Dispatch up to 2; hold the rest in the queue. The moment one of the 2 comes to rest, **immediately pop the next** and dispatch it — you should already know what it is before the slot opens (never an idle slot, never a third agent).
- **Follow-on enqueues take priority over the standing queue.** When a finishing agent's workflow *produces* its own next agent — a build PR that now needs its `quality-assurance-agent` gate, a QA FAIL that needs a re-dispatch, a stale-gated-SHA that needs a re-gate — that **follow-on is enqueued ahead of** anything popped from the standing backlog. Completing an in-flight chain beats starting new work: drain what's already moving before feeding the runway more.

**Why 2 (tightened from the old ~3–4).** iteration-0007's 2× fan-out fired ~7 concurrent heavy agents and tripped a transient **server-side API rate-limit that killed them all mid-run**; the session that set this cap then lost a `project-manager-agent` to the **same silent mid-run death at only 3 concurrent heavy agents** — below the old ~3–4 guidance. 2 is the safe ceiling: it keeps a second slot for the follow-on chains above without re-courting the rate-limit death.
...
- **GitHub issue mechanics → `github-agent`.** **ALL GitHub issue mechanics — creating issues AND editing/updating existing ones — route through `github-agent`; the main thread does NOT file or edit issues inline, not even a quick one-off.** Dispatch `github-agent` to **create + label + dedupe** issues (it runs a dedupe scan first and applies the taxonomy above), to **edit/update an existing issue**, to produce the **Now/Later + effort backlog summary**, and to **rebase stale/behind branches**. The caller (main thread or PM) supplies the intent/prose; the agent handles the GitHub conventions. It never merges, never closes issues, never pushes to `main`. When issue work comes up, **enqueue it as a task and dispatch `github-agent` for it under the standing subagent concurrency cap** (see the "Concurrency cap" section under Orchestration — the at-most-2-subagents-with-a-standing-queue rule) rather than reaching for `gh` inline. (PR *creation* is not its job yet — see its definition.)
...
## The pre-merge gate (quality-assurance-agent)

After **any** push or PR, a **pre-merge quality gate** runs to completion — never assume green. **This is always delegated to the `quality-assurance-agent`; the main thread does not check CI, run `bin/ci-watch` / `bin/ci-screenshots`, or review screenshots itself.** One dispatch returns **one verdict — PASS / FAIL / BLOCKED** — across three facets it self-scopes by PR type: **CI** (watches the run to terminal, root-cause on failure — this absorbs what `ci-monitor` did), the **visual/screenshot review** (vision-capable; the PR-type branching below), and a **per-PR DB-persistence check** for new/changed calculators. It keeps CI logs, screenshot **images**, and DB dumps out of the main context.

This applies to **every** PR, **including one opened by a dispatched agent** (a build agent's, the historian's `docs/journey/` PR, the PM's) — the agent that opens a PR does not gate its own work, so the **orchestrator** dispatches `quality-assurance-agent` the moment a PR exists. Do not let an agent-opened PR slip past ungated. (`quality-assurance-agent` is a registered agent type — dispatch it by name.)

- **The main thread MUST NOT run `bin/ci-watch` or `bin/ci-screenshots` itself** — there is no "quick single-shot glance" exception; one QA dispatch is the cheaper, cleaner default, and a judgment-call carve-out just re-opens the loophole. Those scripts are the QA agent's tools (`bin/ci-watch`: `0`=pass `1`=fail `2`=pending `3`=no-run; `bin/ci-screenshots`: count + sha256 manifest + `--baseline` diff).
- **Visual gate — never skipped, executed inside the verdict.** The QA agent branches by whether the PR can move a pixel: a **UI-surface** PR (touches `app/views` / `app/helpers` / `app/assets` / `app/javascript` / a layout, **or the math of an existing built calculator**) has its CHANGED/NEW shots **viewed and reviewed against `docs/DESIGN.md`**; a **non-UI** PR is **baseline-diffed** to prove no pixel moved. Every shot is accounted for — never a sample. (The non-UI lane is confounded by capture non-determinism — **#87** — so the QA agent adjudicates surprise diffs by eye and says so; known gap.)
- **Human visibility.** Delegating vision keeps image tokens out of the main thread — but also out of *your* view. When you want to eyeball a UI PR's render yourself, ask the QA agent to surface the key shots, or open the PR's artifact. (Auto-posting shots to the PR is a pending refinement — #169.)
- **Death recovery:** if the QA agent dies mid-run without a verdict (the #140 failure mode), the orchestrator re-dispatches a fresh one.
- **A push after the gate invalidates the gate — re-gate before merging.** The QA verdict is pinned to the **SHA it gated**. If a build agent pushes a fix *after* its gate has run (iteration-0007 #272), the gated SHA is now stale and the green verdict no longer describes the PR head. The orchestrator MUST **re-gate** — re-confirm CI is green on the **new head** (a fresh QA dispatch, or at minimum its CI facet) — before merging. Never merge a head the gate did not see.
- **Never auto-merge.** A green gate merges only on explicit human instruction.

> **Adoption note (#169) — proved out; `ci-monitor` now deprecated.** The QA agent was adopted **in `ci-monitor`'s stead** and has now **proved out**: across iteration-0007 it gated all ~26 PRs with **zero deaths**, so its CI facet is the **sole CI gate** and **`ci-monitor` is deprecated** as the CI watcher (kept, not deleted, per rule #2's deprecate-never-delete ethos — see `.claude/agents/ci-monitor.md`). There is no longer a "revert to `ci-monitor` + main-thread visual gate" fallback; QA's three facets are the gate. Open refinements still tracked in **#169**: the DB facet is currently *opportunistic* (often `N/A` unless the dev DB is populated — possibly rehome the persistence check to a CI test), the result format becoming a fixed table, shot-posting to the PR, and the **opus + vision cost on every PR** (accepted for now).
