...
- **`github-agent`** (Sonnet) — GitHub mechanics. Creates issues (applies labels + dedupe scan), creates PRs (the caller writes title/body; it applies labels), serves an issue's spec/state on demand, summarizes the backlog (Now/Later + effort), and rebases stale/behind branches so CI re-runs. The caller supplies intent/prose; it handles GitHub conventions. Never writes app code, agent definitions, or docs; never merges; never pushes to `main`. (Report/action-only — no worktree.)
