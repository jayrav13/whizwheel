...
// Mortgage page (spec issue #187, DESIGN.md §4 "Charts"): the result carries two charts and
// a long P&I amortization schedule (a 30-year mortgage is 360 rows). This controller is pure
...
//     the monthly-payment-component donut (P&I vs. tax vs. insurance vs. HOA, hover tooltips)
//     — into their canvas targets, then hides the static fallbacks. If the JS fails to load
//     the fallbacks stay, so the breakdown is never gated on JS (and never colour-alone — §6).
//
//   • Schedule  — without JS every row renders inside a scroll-capped box (the baseline).
//     With JS the cap is dropped and the controller paginates instead: the first page of rows
//     shows and a "Show all" button reveals the rest. Far easier to scan than a trapped box.
//
...
// calculation re-draws its charts and re-paginates its new schedule.
export default class extends Controller {
  ...
  static values = {
    donut: Array,   // [{ amount, label, color }, …] — the payment-component slices, ordered
  ...
  connect() {
    ...
    try { this.drawDonut() } catch (e) { console.error("mortgage donut:", e) }
    try { this.drawCurve() } catch (e) { console.error("mortgage curve:", e) }
    this.setupSchedule()
  }
  ...
  // ── Donut: the monthly-payment components (Chart.js, hover tooltips) ────────
  drawDonut() {
  ...
  drawCurve() {
    ...
    // bare integer `time` as a UNIX timestamp (which renders "1970"/"1" on the axis), so we
    // format both the axis ticks and the crosshair/tooltip label as the loan YEAR (month/12)
    // — the honest loan-timeline label, not an epoch date. (Backend owns the numbers; this is
    ...
    // times; the elapsed-month index serves as that ordinal time (formatted to the loan year
    // above). The backend already emits month 0 + yearly samples in order.
