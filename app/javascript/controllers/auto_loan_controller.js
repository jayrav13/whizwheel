...
// Auto Loan page (spec issue #251, DESIGN.md §4 "Charts"): the result carries two charts
// (amount-financed-vs-interest donut + balance-over-time curve) and the month-by-month
...
//     hover-capable charts DESIGN.md §4 requires — TradingView lightweight-charts for the
//     "Balance over time" line/time-series (crosshair + per-point tooltip), and Chart.js for
//     the amount-financed-vs-interest donut (hover tooltips) — into their canvas targets,
//     then hides the static fallbacks. If the JS fails to load the fallbacks stay, so the
//     breakdown is never gated on JS (and never colour-alone — §6).
//
...
//     rows shows and a "Show all" button reveals the rest.
//
...
// server already rendered. connect() runs each time Turbo replaces the #result fragment, so
// a fresh calculation re-draws its charts and re-paginates its new schedule. This mirrors
// the Amortization controller's shape (the same chart + pagination enhancement).
export default class extends Controller {
  ...
  connect() {
    ...
    try { this.drawDonut() } catch (e) { console.error("auto-loan donut:", e) }
    try { this.drawCurve() } catch (e) { console.error("auto-loan curve:", e) }
    this.setupSchedule()
  }
  ...
  // ── Donut: amount financed vs. total interest (Chart.js, hover tooltips) ────
  drawDonut() {
    if (!this.hasDonutCanvasTarget) return

    // Reveal the canvas wrapper BEFORE constructing so Chart.js can measure a sized box (a
    // display:none parent reports 0×0). The fallback is retired only AFTER the chart is
    // built (below), so a construction failure leaves the static fallback in place.
  ...
  drawCurve() {
    ...
    // The series time is the elapsed-MONTH index (0, 1, 2, …). lightweight-charts reads a
    // bare integer `time` as a UNIX timestamp (which would render "1970" on the axis), so we
    // format both the axis ticks and the crosshair/tooltip label as the loan MONTH — the
    // honest loan-timeline label, not an epoch date. (Backend owns the numbers; this is
    ...
    // loan month above). The backend already emits month 0 + monthly samples in order.
  ...
  // the attribute nor a utility class can keep it collapsed at chart-construction time (a
